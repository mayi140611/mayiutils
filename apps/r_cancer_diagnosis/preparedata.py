...
def prepareTrainData2(filepath, imgsize=128, maxSlicesNum=587):
    """
    准备训练数据，含train_set, train_label
    ...
    for f in ['train1_label.csv', 'train2_label.csv']:
        ...
        for line in df.itertuples():
            ...
            train_label.append(line[2])
            if count1 == 199:
                np.savez_compressed('batchs2/trainsets_batch{}.npz'.format(count2), train_set=np.array(train_set), train_label=np.array(train_label))
                count2 += 1
                count1 = 0
                train_label = list()
                train_set = list()
    ...
    if train_set:
        print(len(train_set))
        np.savez_compressed('batchs2/trainsets_batch{}.npz'.format(count2), train_set=np.array(train_set),
                            train_label=np.array(train_label))

def getBatch():
    arr = np.arange(64).reshape((16, 4))
    for i in range(16):
        yield arr[i:]
